...
class DigitalSignal {
 ...
 public:
  ...
  bool high() const {
    return state_ == State::HIGH || state_ == State::RISING;
  }
  bool low() const {
    return state_ == State::LOW || state_ == State::FALLING;
  }
  bool rising() const {
    return state_ == State::RISING;
  }
  bool falling() const {
  ...
  void operator=(const bool new_state) {
    this->Process(new_state);
  }
};
